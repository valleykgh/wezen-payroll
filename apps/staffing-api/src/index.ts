...
function getPasswordResetBaseUrl() {
  return (
    process.env.STAFFING_WEB_BASE_URL ||
  ...
  resolvedPayRateCents = parsed.data.payRateCents;
    ...
    try {
      ...
      if (facilityForAlerts) {
        ...
        function zip5(value?: string | null) {
        ...
        const matchingWorkers = workers.filter((worker) => {
          ...
          if (workerZipCoord && facilityZipCoord) {
            const distance = calculateDistanceMiles(
              workerZipCoord.latitude,
              workerZipCoord.longitude,
              facilityZipCoord.latitude,
              facilityZipCoord.longitude
            );
        ...
        await Promise.all(
          matchingWorkers.map(async (worker) => {
...
app.post('/api/facility/availability-invitations', requireRole('FACILITY_ADMIN', 'FACILITY_STAFF'), async (req: AuthedRequest, res) => {
  try {
    ...
    let payRateCents: number | undefined = undefined;
    if (role === 'CNA') payRateCents = facility.defaultCnaRateCents ?? undefined;
    if (role === 'LVN') payRateCents = facility.defaultLvnRateCents ?? undefined;
    if (role === 'RN') payRateCents = facility.defaultRnRateCents ?? undefined;
...
if (String(existing.reviewNotes || '').includes('Cancellation denied by facility')) {
  return res.status(400).json({
    ...
    const workerName =
      `${existing.professional.user.firstName || ''} ${existing.professional.user.lastName || ''}`.trim() ||
...
app.post('/api/worker/documents/upload', upload.single('file'), requireRole('PROFESSIONAL'), async (req: AuthedRequest, res) => {
  try {
    ...
    const uploadedName =
      originalExt && !path.extname(rawUploadedName)
...
app.post('/api/admin/workers/:professionalId/reset-password', requireRole('INTERNAL_ADMIN'), async (req: AuthedRequest, res) => {
...
async function provisionApprovedWorkerInPayroll(professionalId: string) {
  ...
  const response = await fetch(`${apiUrl}/api/integrations/staffing/employee`, {
    ...
    body: JSON.stringify({
      ...
      title: worker.role,
      hourlyRateCents: worker.regularPayRateCents ?? worker.hourlyRateCents,
...
app.get('/api/admin/audit-logs', requireRole('INTERNAL_ADMIN'), async (req: AuthedRequest, res) => {
  try {
...
app.get('/api/facility/workers', requireRole('FACILITY_ADMIN'), async (req: AuthedRequest, res) => {
  try {
    ...
    const favoriteRows = await prisma.$queryRaw<Array<{ professionalId: string }>>`
...
app.post('/api/facility/favorites/:professionalId', requireRole('FACILITY_ADMIN'), async (req: AuthedRequest, res) => {
  try {
    const professionalId = String(req.params.professionalId || '');
    const userId = req.authUser!.userId;
    const facilityId = await getFacilityIdForUser(userId);
...
app.delete('/api/facility/favorites/:professionalId', requireRole('FACILITY_ADMIN'), async (req: AuthedRequest, res) => {
  try {
    const professionalId = String(req.params.professionalId || '');
    const userId = req.authUser!.userId;
    const facilityId = await getFacilityIdForUser(userId);
...
function parseShiftStartDateTime(shiftDate: Date, startTimeLabel: string) {
  const base = new Date(shiftDate);
...
app.post('/api/internal/jobs/send-shift-reminders', async (req, res) => {
  try {
    ...
    for (const request of approvedRequests) {
      try {
        const shiftStart = parseShiftStartDateTime(
          request.shift.date,
          request.shift.startTimeLabel
        );
