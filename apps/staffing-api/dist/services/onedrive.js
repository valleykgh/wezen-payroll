...
export async function getOneDriveInfo() {
    ...
    const response = await axios.get(`${GRAPH_BASE_URL}/users/${encodeURIComponent(userPrincipalName)}/drive`, {
        headers: {
...
export async function listOneDriveRootChildren() {
    const accessToken = await getMicrosoftGraphAccessToken();
    const userPrincipalName = getRequiredEnv('MS_ONEDRIVE_USER');
    const response = await axios.get(`${GRAPH_BASE_URL}/users/${encodeURIComponent(userPrincipalName)}/drive/root/children`, {
        headers: {
            Authorization: `Bearer ${accessToken}`,
        },
    });
...
async function ensureFolderByPath(folderPath) {
    const accessToken = await getMicrosoftGraphAccessToken();
    ...
    for (const part of normalizedPath) {
        ...
        try {
            await axios.get(`${GRAPH_BASE_URL}${rootPrefix}:/${encodeURIComponent(nextPath)}`
                .replace(/%2F/g, '/'), {
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                },
...
export async function getOneDriveDownloadUrl(itemId) {
    const accessToken = await getMicrosoftGraphAccessToken();
    const userPrincipalName = getRequiredEnv('MS_ONEDRIVE_USER');
    const response = await axios.get(`${GRAPH_BASE_URL}/users/${encodeURIComponent(userPrincipalName)}/drive/items/${encodeURIComponent(itemId)}`, {
        headers: {
            Authorization: `Bearer ${accessToken}`,
        },
    });
...
export async function downloadOneDriveFileBuffer(itemId) {
