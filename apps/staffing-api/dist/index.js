...
app.post('/api/worker/shift-invitations/:id/respond', requireRole('PROFESSIONAL'), async (req, res) => {
    try {
        ...
        const workerName = [invitation.professional.user.firstName, invitation.professional.user.lastName]
            .filter(Boolean)
...
app.post('/api/admin/workers/:professionalId/documents/upload', upload.single('file'), requireRole('INTERNAL_ADMIN'), async (req, res) => {
    try {
        const professionalId = String(req.params.professionalId || '');
...
async function sendMessageToProfessional(params) {
    ...
    const workerName = `${professional.user.firstName || ''} ${professional.user.lastName || ''}`.trim() ||
        workerEmail ||
        'Professional';
    await createWorkerNotification({
        professionalId: professional.id,
...
app.post('/api/admin/workers/:professionalId/message', requireRole('INTERNAL_ADMIN'), async (req, res) => {
