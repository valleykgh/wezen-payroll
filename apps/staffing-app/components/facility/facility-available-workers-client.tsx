...
type AvailableWorker = {
  ...
  availabilities: Array<{ date: string; shiftType: ShiftType; note?: string | null }>;
};
...
export function FacilityAvailableWorkersClient() {
  ...
  async function inviteSelectedWorkers() {
    try {
      ...
      if (!selectedWorkerIds.length) throw new Error('Select at least one worker.');
    ...
    <div className="grid gap-4">
      ...
      {workers.length > 0 ? (
        <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
          <div className="grid gap-3">
            ...
            <button type="button" onClick={inviteSelectedWorkers} disabled={busy || selectedWorkerIds.length === 0} className="rounded-2xl bg-slate-950 px-4 py-4 text-sm font-extrabold text-white disabled:opacity-60">
      ...
      <div className="grid gap-3">
        {workers.map((worker) => {
          ...
          return (
            <button key={worker.id} type="button" onClick={() => toggleWorker(worker.id)} className={selected ? 'rounded-3xl border-2 border-cyan-700 bg-cyan-50 p-5 text-left shadow-sm' : 'rounded-3xl bg-white p-5 text-left shadow-sm ring-1 ring-slate-200'}>
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={selected ? 'text-xl text-cyan-700' : 'text-xl text-slate-300'}>✓</span>
              </div>

              <div className="mt-4 flex flex-wrap gap-2">
                {worker.availabilities.map((item, index) => (
                  <span key={`${item.date}-${item.shiftType}-${index}`} className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700">
