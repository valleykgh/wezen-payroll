...
async function main() {
  ...
  const badAdjustments = await prisma.payrollAdjustment.findMany({
    where: {
      employeeId,
      payrollRunId: run.id,
  ...
  if (newAdjustmentsCents < 0) {
    throw new Error(
      `Repair would make adjustmentsCents negative. Current=${runEmp.adjustmentsCents}, remove=${duplicateAdjustmentCents}`
    );
  }
