...
import { calculatePayCentsWithRule, getHolidayRule } from "./_shared";
...
const existingCorrection = await prisma.payrollCorrection.findFirst({
  where: {
...
router.get(
  "/payroll-runs/:runId/snapshots/:snapshotId/calc",
  async (req, res) => {
    try {
      const snapshotId = String(req.params.snapshotId || "");

      const snapshot = await prisma.payrollRunEntrySnapshot.findUnique({
        where: { id: snapshotId },
        include: {
          employee: {
            select: {
      ...
      const data: any = snapshot.snapshotJson || {};
...
router.post(
  "/payroll-runs/:runId/snapshots/:snapshotId/correct",
  async (req, res) => {
    try {
