...
router.post("/payroll-repair/fix-double-counted-time-entry-pay-now", async (req, res) => {
  try {
    ...
    const runEmp = run.employees.find(
      (r) => String(r.employee?.email || "").toLowerCase() === employeeEmail
    );
    ...
    const attachedAdjustments = await prisma.payrollAdjustment.findMany({
      where: {
        employeeId,
        payrollRunId: run.id,
        workDate: {
    ...
    const duplicateAdjustmentCents = badAdjustments.reduce(
    ...
    const newAdjustmentsCents =
      Number(runEmp.adjustmentsCents || 0) - duplicateAdjustmentCents;
    const newNetPayCents =
      Number(runEmp.netPayCents || 0) - duplicateAdjustmentCents;
    ...
    const repaired = await prisma.payrollRunEmployee.findUnique({
      where: { id: runEmp.id },
      select: {
        grossPayCents: true,
