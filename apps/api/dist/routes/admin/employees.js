...
router.get("/employees/:id/ledger", async (req, res) => {
    try {
        ...
        const ledger = await prisma_1.prisma.employeePayrollLedger.findMany({
            where: {
                employeeId,
                periodEnd: {
                    gte: fromDt,
        ...
        const payrollRuns = await prisma_1.prisma.payrollRunEmployee.findMany({
            where: {
                employeeId,
                payrollRun: {
                    periodStart: {
        ...
        const payrollTotals = payrollRuns.reduce((acc, r) => {
        ...
        const ledgerTotals = ledger.reduce((acc, r) => {
        ...
        const totalGrossPayCents = payrollTotals.grossPayCents + ledgerTotals.earningsAdjustmentCents;
        const totalNetPayCents = payrollTotals.netPayCents + ledgerTotals.earningsAdjustmentCents;
        const totals = {
