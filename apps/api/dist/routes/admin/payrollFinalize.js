...
router.post("/payroll-runs/finalize", async (req, res) => {
    try {
        ...
        for (const e of entries) {
            ...
            if (!facId) {
                validationErrors.push(`Employee "${employeeName}" has a time entry on ${fmtISODateOnly(e.workDate)} with no facility.`);
                continue;
            }
            const facilityName = e.facility?.name || facId;
            const applicableRates = facilityRates.filter((r) => String(r.facilityId || "") === facId);
            const effectiveRate = findEffectiveFacilityRate(applicableRates, title, new Date(e.workDate));
            if (!effectiveRate) {
                validationErrors.push(`Missing billing rate for facility "${facilityName}", title "${title}", work date ${fmtISODateOnly(e.workDate)}.`);
            }
        }
