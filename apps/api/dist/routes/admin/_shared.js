...
exports.HOLIDAY_DATES = exports.MAX_GAP_MINUTES = exports.SHIFT_TYPE = exports.TIME_ENTRY_STATUS = void 0;
exports.requireAdminPinFromBody = requireAdminPinFromBody;
exports.hasValidAdminPin = hasValidAdminPin;
...
exports.isHolidayDate = isHolidayDate;
exports.getHolidayMultiplier = getHolidayMultiplier;
exports.calculatePayCents = calculatePayCents;
exports.calculateBillCents = calculateBillCents;
exports.sumBreakMinutesFromEntry = sumBreakMinutesFromEntry;
exports.getHolidayRule = getHolidayRule;
exports.calculatePayCentsWithRule = calculatePayCentsWithRule;
exports.calculateBillCentsWithRule = calculateBillCentsWithRule;
exports.getBillingRun = getBillingRun;
exports.getOrCreateBillingRun = getOrCreateBillingRun;
exports.getActiveHolidayDates = getActiveHolidayDates;
...
exports.getBillableWorkRows = getBillableWorkRows;
...
function isHolidayDate(workDate) {
    return exports.HOLIDAY_DATES.has(dateOnlyUTC(workDate));
}
function getHolidayMultiplier(workDate) {
    return isHolidayDate(workDate) ? 1.5 : 1;
}
function calculatePayCents(params) {
    const { regularMinutes, overtimeMinutes, doubleMinutes, hourlyRateCents, workDate, } = params;
    const holidayMultiplier = getHolidayMultiplier(workDate);
    const regularPayCents = Math.round((regularMinutes * hourlyRateCents * holidayMultiplier) / 60);
    const overtimePayCents = Math.round((overtimeMinutes * hourlyRateCents * 1.5) / 60);
    const doublePayCents = Math.round((doubleMinutes * hourlyRateCents * 2) / 60);
...
function calculateBillCents(params) {
    const { regularMinutes, overtimeMinutes, doubleMinutes, regRateCents, otRateCents, dtRateCents, workDate, } = params;
    const holidayMultiplier = getHolidayMultiplier(workDate);
    return Math.round((regularMinutes * regRateCents * holidayMultiplier +
        overtimeMinutes * otRateCents +
        doubleMinutes * dtRateCents) / 60);
}
...
async function getHolidayRule(workDate) {
...
function calculatePayCentsWithRule(params) {
    const { regularMinutes, overtimeMinutes, doubleMinutes, hourlyRateCents, holidayRule, } = params;
    const regularMultiplier = holidayRule.isHoliday && holidayRule.appliesToRegularOnly
        ? Number(holidayRule.payMultiplier || 1.5)
        : 1;
    const regularPayCents = Math.round((regularMinutes * hourlyRateCents * regularMultiplier) / 60);
    const overtimePayCents = Math.round((overtimeMinutes * hourlyRateCents * 1.5) / 60);
    const doublePayCents = Math.round((doubleMinutes * hourlyRateCents * 2) / 60);
    const grossPayCents = regularPayCents + overtimePayCents + doublePayCents;
    return {
...
function calculateBillCentsWithRule(params) {
    const { regularMinutes, overtimeMinutes, doubleMinutes, regRateCents, otRateCents, dtRateCents, holidayRule, } = params;
    const regularMultiplier = holidayRule.isHoliday && holidayRule.appliesToRegularOnly
        ? Number(holidayRule.billMultiplier || 1.5)
        : 1;
    return Math.round((regularMinutes * regRateCents * regularMultiplier +
        overtimeMinutes * otRateCents +
        doubleMinutes * dtRateCents) / 60);
}
async function getBillingRun(params) {
    const { facilityId, periodStart, periodEnd, invoiceType } = params;
    return prisma_1.prisma.billingRun.findFirst({
        where: {
            facilityId,
            periodStart,
            periodEnd,
...
async function getOrCreateBillingRun(params) {
    const { facilityId, periodStart, periodEnd, invoiceType, invoiceNumber } = params;
    const existing = await prisma_1.prisma.billingRun.findFirst({
        where: {
            facilityId,
            periodStart,
            periodEnd,
    ...
    return prisma_1.prisma.billingRun.create({
        data: {
            facilityId,
            periodStart,
            periodEnd,
...
async function assertEditableNotLocked(timeEntryId, req) {
...
async function getBillableWorkRows(params) {
    ...
    for (const e of timeEntries) {
        const breakMinutes = Array.isArray(e.breaks)
            ? e.breaks.reduce((sum, b) => sum + Number(b.minutes || 0), 0)
        ...
        rows.push({
            ...
            sourceId: e.id,
            billedAt: e.billedAt || null,
            invoiceNumber: e.invoiceNumber || null,
            invoiceType: e.invoiceType || null,
            ...
            doubleMinutes: buckets.doubleMinutes,
            breaks: e.breaks || [],
            breaksJson: Array.isArray(e.breaks) && e.breaks.length > 0
                ? e.breaks.map((b) => ({
                    startTime: b.startTime,
                    endTime: b.endTime,
                }))
                : (e.breaksJson || null),
            punchesJson: e.punchesJson || null,
            notes: e.notes || null,
        });
    }
    for (const a of adjustments) {
        ...
        rows.push({
            ...
            sourceId: a.id,
            billedAt: a.billedAt || null,
            invoiceNumber: a.invoiceNumber || null,
            invoiceType: a.invoiceType || null,
            ...
            doubleMinutes,
            breaks: [],
            punchesJson: a.punchesJson || null,
            breaksJson: a.breaksJson || null,
            notes: a.reason || null,
            amountCents: a.amountCents || 0,
            billAmountCents: a.billAmountCents || null,
        });
    }
    return rows;
}
