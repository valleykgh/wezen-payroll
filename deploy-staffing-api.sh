...
SERVICE_NAME="wezen-staffing-api-service"
TASK_FAMILY="wezen-staffing-api-task"
CONTAINER_NAME="staffing-api"
ECR_REPO="wezen-staffing-api"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TASK_DEF_FILE="$SCRIPT_DIR/wezen-staffing-api-task-def.json"
...
echo "2) Building and pushing staffing-api image for linux/amd64..."
docker buildx build \
  --platform linux/amd64 \
  -f docker/staffing-api.Dockerfile \
  -t "$IMAGE_URI" \
  --push \
  .
...
echo
echo "Staffing API deploy complete."
